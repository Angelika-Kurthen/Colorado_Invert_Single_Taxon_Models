############################
# Temperature % increase x HFE BAET
#############################
...
BAET_temp_abund_HFE <- data.frame(abundance=numeric(), temperature=factor(), taxa=factor())
BAET_temp_biomass_HFE <- data.frame(biomass=numeric(), temperature = factor(), taxa = factor())
BAET_temp_percapita_HFE <- data.frame(percapita=numeric(), temperature = factor(), taxa = factor())
...
results <- mclapply(temp_seq, function(te) {
  ...
  colnames(average_size) <- colnames(BAET_temp_biomass_HFE)
  # put the percapita biomass of each stage in a dataframe
  #s1pc <- (0.0053 * (mean(sizes[-c(1:260)]/2, na.rm = T))^2.875)
  #s2pc <- (0.0053 * (mean(sizes[-c(1:260)], na.rm = T))^2.875)
  s3pc <- (0.0053 * (mean(sizes[-c(1:260)], na.rm = T))^2.875)
  percapita <- c(s3pc)
  ...
  colnames(percapita_biomass) <- colnames(BAET_temp_percapita_HFE)
  ...
  colnames(average_means) <- colnames(BAET_temp_abund_HFE)
  # Append results to the main data storage
  BAET_temp_abund_HFE <- rbind(BAET_temp_abund_HFE, average_means)
  BAET_temp_biomass_HFE <- rbind(BAET_temp_biomass_HFE, average_size)
  BAET_temp_percapita_HFE <- rbind(BAET_temp_percapita_HFE, percapita_biomass)
  
  # Return results as a list
  return(list(BAET_temp_abund_HFE = average_means, BAET_temp_biomass_HFE = average_size, BAET_temp_percapita_HFE = percapita_biomass))
}, mc.cores = detectCores() - 1)

# Combine results from all temperature scenarios into final dataframes
BAET_temp_abund_HFE <- do.call(rbind, lapply(results, `[[`, "BAET_temp_abund_HFE"))
BAET_temp_biomass_HFE <- do.call(rbind, lapply(results, `[[`, "BAET_temp_biomass_HFE"))
BAET_temp_percapita_HFE <- do.call(rbind, lapply(results, `[[`, "BAET_temp_percapita_HFE"))

# Write results to CSV files
write.csv(BAET_temp_abund_HFE, "BAET_temp_abund_HFE.csv", row.names = FALSE)
write.csv(BAET_temp_biomass_HFE, "BAET_temp_biomass_HFE.csv", row.names = FALSE)
write.csv(BAET_temp_percapita_HFE, "BAET_temp_percapita_HFE.csv", row.names = FALSE)


# tabula rasa
rm(temp)
rm(temps)
rm(BAET_temp_abund_HFE)
rm(BAET_temp_biomass_HFE)
...
# makes some vectors for data to go into
BAET_temp_abund_HFE_spike <- data.frame(abundance=numeric(), temperature=factor(), taxa=factor())
BAET_temp_biomass_HFE_spike <- data.frame(biomass=numeric(), temperature = factor(), taxa = factor())
BAET_temp_percapita_HFE_spike <- data.frame(percapita=numeric(), temperature = factor(), taxa = factor())
...
results <- mclapply(temp_seq, function(te) {
  ...
  # Store biomass data in a dataframe
  average_size <- cbind(sizes_list, rep(te, times = length(sizes_list)), rep("BAET",times = length(sizes_list)))
  colnames(average_size) <- colnames(BAET_temp_biomass_HFE_spike)
  # put the percapita biomass of each stage in a dataframe
  #s1pc <-  (0.0053 * (mean(sizes[expanded_HFE_rows]/2, na.rm = T))^2.875)
  #s2pc <- (0.0053 * (mean(sizes[expanded_HFE_rows], na.rm = T))^2.875)
  s3pc <-(0.0053 * (mean(sizes[expanded_HFE_rows], na.rm = T))^2.875)
  percapita <- c(s3pc)
  
  # store percapita biomass data in a dataframe
  percapita_biomass <- cbind(percapita, rep(te, times = length(percapita)), rep("BAET",times = length(percapita)))
  colnames(percapita_biomass) <- colnames(BAET_temp_percapita_HFE_spike)
  ...
  # Store abundance data in a dataframe
  average_means <- cbind(means, rep(te, times = length(means)), rep("BAET",times = length(means)))
  colnames(average_means) <- colnames(BAET_temp_abund_HFE_spike)
  # Append results to the main data storage
  BAET_temp_abund_HFE_spike <- rbind(BAET_temp_abund_HFE_spike, average_means)
  BAET_temp_biomass_HFE_spike <- rbind(BAET_temp_biomass_HFE_spike, average_size)
  BAET_temp_percapita_HFE_spike <- rbind(BAET_temp_percapita_HFE_spike, percapita_biomass)
  
  # Return results as a list
  return(list(BAET_temp_abund_HFE_spike = average_means, BAET_temp_biomass_HFE_spike = average_size, BAET_temp_percapita_HFE_spike = percapita_biomass))
}, mc.cores = detectCores() - 1)

# Combine results from all temperature scenarios into final dataframes
BAET_temp_abund_HFE_spike <- do.call(rbind, lapply(results, `[[`, "BAET_temp_abund_HFE_spike"))
BAET_temp_biomass_HFE_spike <- do.call(rbind, lapply(results, `[[`, "BAET_temp_biomass_HFE_spike"))
BAET_temp_percapita_HFE_spike <- do.call(rbind, lapply(results, `[[`, "BAET_temp_percapita_HFE_spike"))

# Write results to CSV files
write.csv(BAET_temp_abund_HFE_spike, "BAET_temp_abund_HFE_spike.csv", row.names = FALSE)
write.csv(BAET_temp_biomass_HFE_spike, "BAET_temp_biomass_HFE_spike.csv", row.names = FALSE)
write.csv(BAET_temp_percapita_HFE_spike, "BAET_temp_percapita_HFE_spike.csv", row.names = FALSE)
