############################
# Temperature % increase + hydropeaking BAET
#############################
...
BAET_temp_hyd_abund <- data.frame(abundance=numeric(), temperature=factor(), taxa=factor())
BAET_temp_hyd_biomass<- data.frame(biomass=numeric(), temperature = factor(), taxa = factor())
results <- mclapply(temp_seq, function(te) {
  ...
  colnames(average_size) <- colnames(BAET_temp_hyd_biomass)
  ...
  colnames(average_means) <- colnames(BAET_temp_hyd_abund)
  # Append results to the main data storage
  BAET_temp_hyd_abund <- rbind(BAET_temp_hyd_abund, average_means)
  BAET_temp_hyd_biomass <- rbind(BAET_temp_hyd_biomass, average_size)
  # Return results as a list
  return(list(BAET_temp_hyd_abund = average_means, BAET_temp_hyd_biomass = average_size))
}, mc.cores = detectCores() - 1)

# Combine results from all temperature scenarios into final dataframes
BAET_temp_hyd_abund <- do.call(rbind, lapply(results, `[[`, "BAET_temp_hyd_abund"))
BAET_temp_hyd_biomass <- do.call(rbind, lapply(results, `[[`, "BAET_temp_hyd_biomass"))
...
BAET_temp_hyd_biomass_spike <- data.frame(biomass=numeric(), temperature = factor(), taxa = factor())
results <- mclapply(temp_seq, function(te) {
  set.seed(123) # make reproducible
  # model sizes
  temps$Temperature <- temps$Temperature * te
  ...
  # Store biomass data in a dataframe
  average_size <- cbind(sizes_list, rep(te, times = length(sizes_list)), rep("BAET",times = length(sizes_list)))
  colnames(average_size) <- colnames(BAET_temp_hyd_biomass_spike)
  ...
  # Store abundance data in a dataframe
  average_means <- cbind(means, rep(te, times = length(means)), rep("BAET",times = length(means)))
  colnames(average_means) <- colnames(BAET_temp_hyd_abund_spike)
  # Append results to the main data storage
  BAET_temp_hyd_abund_spike <- rbind(BAET_temp_hyd_abund_spike, average_means)
  BAET_temp_hyd_biomass_spike <- rbind(BAET_temp_hyd_biomass_spike, average_size)
  # Return results as a list
  return(list(BAET_temp_hyd_abund_spike = average_means, BAET_temp_hyd_biomass_spike = average_size))
}, mc.cores = detectCores() - 1)

# Combine results from all temperature scenarios into final dataframes
BAET_temp_hyd_abund_spike <- do.call(rbind, lapply(results, `[[`, "BAET_temp_hyd_abund_spike"))
BAET_temp_hyd_biomass_spike <- do.call(rbind, lapply(results, `[[`, "BAET_temp_hyd_biomass_spike"))

# Write results to CSV files
write.csv(BAET_temp_hyd_abund_spike, "BAET_temp_hyd_abund_spike.csv", row.names = FALSE)
write.csv(BAET_temp_hyd_biomass_spike, "BAET_temp_hyd_biomass_spike.csv", row.names = FALSE)
