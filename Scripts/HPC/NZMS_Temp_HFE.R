...
NZMS_temp_abund_HFE <- data.frame(abundance=numeric(), temperature=factor(), taxa=factor())
NZMS_temp_biomass_HFE <- data.frame(biomass=numeric(), temperature = factor(), taxa = factor())
NZMS_temp_percapita_HFE <- data.frame(percapita=numeric(), temperature = factor(), taxa = factor())
results <- mclapply(temp_seq, function(te) {
  ...
  s1pc <- (0.02 * mean(c(0.5, 3.2))^2.4315)
  s2pc <- (0.02 * mean(c(3.2, 4))^2.4315)
  s3pc <- (0.02 * mean(c(4, 5.5))^2.4315)
  percapita <- c(s1pc, s2pc, s3pc)
  
  # store percapita biomass data in a dataframe
  percapita_biomass <- cbind(percapita, rep(te, times = length(percapita)), rep("NZMS",times = length(sizes_list)))
  colnames(percapita_biomass) <- colnames(NZMS_temp_percapita_HFE)
  ...
  colnames(average_size) <- colnames(NZMS_temp_biomass_HFE)
  
  # Append results to the main data storage
  NZMS_temp_abund_HFE <- rbind(NZMS_temp_abund_HFE, average_means)
  NZMS_temp_biomass_HFE <- rbind(NZMS_temp_biomass_HFE, average_size)
  NZMS_temp_percapita_HFE <- rbind(NZMS_temp_percapita_HFE, percapita_biomass)
  # Return results as a list
  return(list(NZMS_temp_abund_HFE = average_means, NZMS_temp_biomass_HFE = average_size, NZMS_temp_percapita_HFE = percapita_biomass))
}, mc.cores = detectCores() - 1)

# Combine results from all temperature scenarios into final dataframes
NZMS_temp_abund_HFE <- do.call(rbind, lapply(results, `[[`, "NZMS_temp_abund_HFE"))
NZMS_temp_biomass_HFE <- do.call(rbind, lapply(results, `[[`, "NZMS_temp_biomass_HFE"))
NZMS_temp_percapita_HFE <- do.call(rbind, lapply(results, `[[`, "NZMS_temp_percapita_HFE"))

# Write results to CSV files
write.csv(NZMS_temp_abund_HFE, "NZMS_temp_abund_HFE.csv", row.names = FALSE)
write.csv(NZMS_temp_biomass_HFE, "NZMS_temp_biomass_HFE.csv", row.names = FALSE)
write.csv(NZMS_temp_percapita_HFE, "NZMS_temp_percapita_HFE.csv", row.names = FALSE)


# tabula rasa
rm(temp)
rm(temps)
rm(NZMS_temp_abund_HFE)
rm(NZMS_temp_biomass_HFE)
...
# makes some vectors for data to go into
NZMS_temp_abund_HFE_spike <- data.frame(abundance=numeric(), temperature=factor(), taxa=factor())
NZMS_temp_biomass_HFE_spike <- data.frame(biomass=numeric(), temperature = factor(), taxa = factor())
NZMS_temp_percapita_HFE_spike <- data.frame(percapita=numeric(), temperature = factor(), taxa = factor())
...
results <- mclapply(temp_seq, function(te) {
  ...
  # Store biomass data in a dataframe
  average_size <- cbind(sizes_list, rep(te, times = length(sizes_list)), rep("NZMS",times = length(sizes_list)))
  colnames(average_size) <- colnames(NZMS_temp_biomass_HFE_spike)
  # put the percapita biomass of each stage in a dataframe
  s1pc <- (0.02 * mean(c(0.5, 3.2))^2.4315)
  s2pc <- (0.02 * mean(c(3.2, 4))^2.4315)
  s3pc <- (0.02 * mean(c(4, 5.5))^2.4315)
  ...
  # store percapita biomass data in a dataframe
  percapita_biomass <- cbind(percapita, rep(te, times = length(percapita)), rep("NZMS",times = length(sizes_list)))
  colnames(percapita_biomass) <- colnames(NZMS_temp_percapita_HFE_spike)
  
  # Append results to the main data storage
  NZMS_temp_abund_HFE_spike <- rbind(NZMS_temp_abund_HFE_spike, average_means)
  NZMS_temp_biomass_HFE_spike <- rbind(NZMS_temp_biomass_HFE_spike, average_size)
  NZMS_temp_percapita_HFE_spike <- rbind(NZMS_temp_percapita_HFE_spike, percapita_biomass)
  
  # Return results as a list
  return(list(NZMS_temp_abund_HFE_spike = average_means, NZMS_temp_biomass_HFE_spike = average_size, NZMS_temp_percapita_HFE_spike = percapita_biomass))
}, mc.cores = detectCores() - 1)

# Combine results from all temperature scenarios into final dataframes
NZMS_temp_abund_HFE_spike <- do.call(rbind, lapply(results, `[[`, "NZMS_temp_abund_HFE_spike"))
NZMS_temp_biomass_HFE_spike <- do.call(rbind, lapply(results, `[[`, "NZMS_temp_biomass_HFE_spike"))
NZMS_temp_percapita_HFE_spike <- do.call(rbind, lapply(results, `[[`, "NZMS_temp_percapita_HFE_spike"))

# Write results to CSV files
write.csv(NZMS_temp_abund_HFE_spike, "NZMS_temp_abund_HFE_spike.csv", row.names = FALSE)
write.csv(NZMS_temp_biomass_HFE_spike, "NZMS_temp_biomass_HFE_spike.csv", row.names = FALSE)
write.csv(NZMS_temp_percapita_HFE_spike, "NZMS_temp_percapita_HFE_spike.csv", row.names = FALSE)
