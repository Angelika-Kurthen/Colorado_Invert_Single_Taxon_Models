...
results <- mclapply(temp_seq, function(te) {
  ...
  colnames(average_size) <- colnames(HYOS_temp_biomass_HFE)
  ...
  colnames(average_means) <- colnames(HYOS_temp_abund_HFE)
...
rm(temp)
rm(temps)
rm(HYOS_temp_abund_HFE)
rm(HYOS_temp_biomass_HFE)
