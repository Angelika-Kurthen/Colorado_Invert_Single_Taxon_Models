############################
# Temperature % + Hyd Sensitivity GAMM
#############################
...
GAMM_temp_hyd_abund <- data.frame(abundance=numeric(), temperature=factor(), taxa=factor())
GAMM_temp_hyd_biomass <- data.frame(biomass=numeric(), temperature = factor(), taxa = factor())
results <- mclapply(temp_seq, function(te) {
  ...
  colnames(average_size) <- colnames(GAMM_temp_hyd_biomass)
  ...
  colnames(average_means) <- colnames(GAMM_temp_hyd_abund)
  # Append results to the main data storage
  GAMM_temp_abund <- rbind(GAMM_temp_hyd_abund, average_means)
  GAMM_temp_biomass <- rbind(GAMM_temp_hyd_biomass, average_size)
  # Return results as a list
  return(list(GAMM_temp_hyd_abund = average_means, GAMM_temp_hyd_biomass = average_size))
}, mc.cores = detectCores() - 1)

# Combine results from all temperature scenarios into final dataframes
GAMM_temp_hyd_abund <- do.call(rbind, lapply(results, `[[`, "GAMM_temp_hyd_abund"))
GAMM_temp_hyd_biomass <- do.call(rbind, lapply(results, `[[`, "GAMM_temp_hyd_biomass"))

# Write results to CSV files
write.csv(GAMM_temp_hyd_abund, "GAMM_temp_hyd_abund.csv", row.names = FALSE)
write.csv(GAMM_temp_hyd_biomass, "GAMM_temp_hyd_biomass.csv", row.names = FALSE)

# tabula rasa
rm(temp)
rm(temps)
rm(GAMM_temp_hyd_abund)
rm(GAMM_temp_hyd_biomass)
...
# makes some vectors for data to go into
GAMM_temp_hyd_abund_spike <- data.frame(abundance=numeric(), temperature=factor(), taxa=factor())
GAMM_temp_hyd_biomass_spike <- data.frame(biomass=numeric(), temperature = factor(), taxa = factor())
results <- mclapply(temp_seq, function(te) {
  set.seed(123) # make reproducible
  # modify temp regime
  temps$Temperature <- temps$Temperature * te
  ...
  # Store biomass data in a dataframe
  average_size <- cbind(sizes_list, rep(te, times = length(sizes_list)), rep("GAMM",times = length(sizes_list)))
  colnames(average_size) <- colnames(GAMM_temp_hyd_biomass_spike)
  ...
  # Store abundance data in a dataframe
  average_means <- cbind(means, rep(te, times = length(means)), rep("GAMM",times = length(means)))
  colnames(average_means) <- colnames(GAMM_temp_hyd_abund_spike)
  # Append results to the main data storage
  GAMM_temp_hyd_abund_spike <- rbind(GAMM_temp_hyd_abund_spike, average_means)
  GAMM_temp_hyd_biomass_spike <- rbind(GAMM_temp_hyd_biomass_spike, average_size)
  # Return results as a list
  return(list(GAMM_temp_hyd_abund_spike = average_means, GAMM_temp_hyd_biomass_spike = average_size))
}, mc.cores = detectCores() - 1)

# Combine results from all temperature scenarios into final dataframes
GAMM_temp_hyd_abund_spike <- do.call(rbind, lapply(results, `[[`, "GAMM_temp_hyd_abund_spike"))
GAMM_temp_hyd_biomass_spike <- do.call(rbind, lapply(results, `[[`, "GAMM_temp_hyd_biomass_spike"))

# Write results to CSV files
write.csv(GAMM_temp_hyd_abund_spike, "GAMM_temp_hyd_abund_spike.csv", row.names = FALSE)
write.csv(GAMM_temp_hyd_biomass_spike, "GAMM_temp_hyd_biomass_spike.csv", row.names = FALSE)
