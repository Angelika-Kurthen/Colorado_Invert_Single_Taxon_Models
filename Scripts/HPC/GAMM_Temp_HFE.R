...
GAMM_temp_abund_HFE <- data.frame(abundance=numeric(), temperature=factor(), taxa=factor())
GAMM_temp_biomass_HFE <- data.frame(biomass=numeric(), temperature = factor(), taxa = factor())
GAMM_temp_percapita_HFE <- data.frame(percapita=numeric(), temperature = factor(), taxa = factor())
...
results <- mclapply(temp_seq, function(te) {
  ...
  colnames(average_size) <- colnames(GAMM_temp_biomass_HFE)
  ...
  colnames(percapita_biomass) <- colnames(GAMM_temp_percapita_HFE)
  ...
  colnames(average_means) <- colnames(GAMM_temp_abund_HFE)
  # Append results to the main data storage
  GAMM_temp_abund_HFE <- rbind(GAMM_temp_abund_HFE, average_means)
  GAMM_temp_biomass_HFE <- rbind(GAMM_temp_biomass_HFE, average_size)
  GAMM_temp_percapita_HFE_HFE <- rbind(GAMM_temp_percapita_HFE, percapita_biomass)
  
  # Return results as a list
  return(list(GAMM_temp_abund_HFE = average_means, GAMM_temp_biomass_HFE = average_size, GAMM_temp_percapita_HFE = percapita_biomass))
}, mc.cores = detectCores() - 1)

# Combine results from all temperature scenarios into final dataframes
GAMM_temp_abund_HFE <- do.call(rbind, lapply(results, `[[`, "GAMM_temp_abund_HFE"))
GAMM_temp_biomass_HFE <- do.call(rbind, lapply(results, `[[`, "GAMM_temp_biomass_HFE"))
GAMM_temp_percapita_HFE <- do.call(rbind, lapply(results, `[[`, "GAMM_temp_percapita_HFE"))

# Write results to CSV files
write.csv(GAMM_temp_abund_HFE, "GAMM_temp_abund_HFE.csv", row.names = FALSE)
write.csv(GAMM_temp_biomass_HFE, "GAMM_temp_biomass_HFE.csv", row.names = FALSE)
write.csv(GAMM_temp_percapita_HFE, "GAMM_temp_percapita_HFE.csv", row.names = FALSE)

# tabula rasa
rm(temp)
rm(temps)
rm(GAMM_temp_abund_HFE)
rm(GAMM_temp_biomass_HFE)
...
# makes some vectors for data to go into
GAMM_temp_abund_HFE_spike <- data.frame(abundance=numeric(), temperature=factor(), taxa=factor())
GAMM_temp_biomass_HFE_spike <- data.frame(biomass=numeric(), temperature = factor(), taxa = factor())
GAMM_temp_percapita_HFE_spike <- data.frame(percapita=numeric(), temperature = factor(), taxa = factor())
...
results <- mclapply(temp_seq, function(te) {
  ...
  # Store biomass data in a dataframe
  average_size <- cbind(sizes_list, rep(te, times = length(sizes_list)), rep("GAMM",times = length(sizes_list)))
  colnames(average_size) <- colnames(GAMM_temp_biomass_HFE_spike)
  # put the percapita biomass of each stage in a dataframe
  s1pc <- (0.063 * mean(c(2.5, 7))^2.46)
  s2pc <- (0.063 * mean(c(7, 9))^2.46)
  s3pc <- (0.063 * mean(c(9, 12))^2.46)
  ...
  # store percapita biomass data in a dataframe
  percapita_biomass <- cbind(percapita, rep(te, times = length(percapita)), rep("GAMM",times = length(percapita)))
  colnames(percapita_biomass) <- colnames(GAMM_temp_percapita_HFE_spike)
  ...
  # Store abundance data in a dataframe
  average_means <- cbind(means, rep(te, times = length(means)), rep("GAMM",times = length(means)))
  colnames(average_means) <- colnames(GAMM_temp_abund_HFE_spike)
  # Append results to the main data storage
  GAMM_temp_abund_HFE_spike <- rbind(GAMM_temp_abund_HFE_spike, average_means)
  GAMM_temp_biomass_HFE_spike <- rbind(GAMM_temp_biomass_HFE_spike, average_size)
  GAMM_temp_percapita_HFE_spike <- rbind(GAMM_temp_percapita_HFE_spike, percapita_biomass)
  
  # Return results as a list
  return(list(GAMM_temp_abund_HFE_spike = average_means, GAMM_temp_biomass_HFE_spike = average_size, GAMM_temp_percapita_HFE_spike = percapita_biomass))
}, mc.cores = detectCores() - 1)

# Combine results from all temperature scenarios into final dataframes
GAMM_temp_abund_HFE_spike <- do.call(rbind, lapply(results, `[[`, "GAMM_temp_abund_HFE_spike"))
GAMM_temp_biomass_HFE_spike <- do.call(rbind, lapply(results, `[[`, "GAMM_temp_biomass_HFE_spike"))
GAMM_temp_percapita_HFE_spike <- do.call(rbind, lapply(results, `[[`, "GAMM_temp_percapita_HFE_spike"))

# Write results to CSV files
write.csv(GAMM_temp_abund_HFE_spike, "GAMM_temp_abund_HFE_spike.csv", row.names = FALSE)
write.csv(GAMM_temp_biomass_HFE_spike, "GAMM_temp_biomass_HFE_spike.csv", row.names = FALSE)
write.csv(GAMM_temp_percapita_HFE_spike, "GAMM_temp_percapita_HFE_spike.csv", row.names = FALSE)
